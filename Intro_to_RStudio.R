...
# For this assingment, you will need the loan50.csv file (see DA 1 on canvas)
...
# You should see an option to Import Dataset. Select this option, then Import. 
...
# Click on loan50 to preview the dataset. 

#### Exporing the Data ####

# The following lines of code make use of some built-in R functions that will 
# help us get familiar with the dataset. Take some time to read the 
# descriptions and run the lines of code.

# List the names of the variables 
names(loan50)

# See a preview of the first 6 rows of the dataset
head(loan50)
...
# Let's focus on the loan amount variable available in this dataset.
# To reference a variable stored in a dataset, we first need to list
# the dataset name, followed by $ and the variable name. For example, 
# if we want to look at the first 6 values of loan_amount in the 
# loan50 dataset, we can run the following line. 
...
# home ownership status of loan applicant (variable homeownership). To do this, we will construct 
# a side-by-side boxplot. Using the boxplot function, we need to specify both
...
# Let's focus now only on the homeownership data. This is a 
# categorical variable containing three categories: mortgage, 
# own, and rent. 

# The following line of code will calculate the number of 
# observations in the dataset that fall in the rent category. 
...
mean(loan50$homeownership == "rent")
